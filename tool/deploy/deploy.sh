...
echo
echo "WordPress Plugin Git-Flow SVN Deploy v1.0.0-dev2 Modified by Michael Uno"
echo 
...
# Set the local working copy directory path
if [ -z "$WORKINGDIR" ] ; then
    WORKINGDIR=`pwd`
fi
WORKINGDIR=$(abspath $WORKINGDIR)

# Set the temporary directory. On Windows use the environment variable.
if [ -z "$TEMP_PATH" ] ; then
    case "$OSTYPE" in
       "msys") TEMP_PATH="$TEMP"
       ;;
...
# The SVN repository base URL.
if [ -z "$SVN_BASE_URL" ] ; then
    SVN_BASE_URL="http://plugins.svn.wordpress.org"
fi

# Set up some default values. Feel free to change these in your own script
default_svnpath="$TEMP_PATH/$PLUGINSLUG"
default_svnurl="$SVN_BASE_URL/$PLUGINSLUG"
default_plugindir="$WORKINGDIR/$PLUGINSLUG"
...
if [ -z "$SVN_USERNAME" ] ; then
    printf "1d) Your WordPress repo SVN username ($SVN_USERNAME): "
    read -e input
    SVNUSER="${input:-$SVN_USERNAME}" # Populate with default if empty
    echo
fi

# SVN ignore list
if [ -z "$SVN_IGNORE_FILE_NAME" ] ; then
SVN_IGNORE_FILE_NAME="svnignore.txt"
fi
SVN_IGNORE_FILE_PATH = "$WORKINGDIR/$SVN_IGNORE_FILE_NAME"
...
svn propset svn:ignore -F "$SVN_IGNORE_FILE_PATH" "$SVNPATH/trunk/"
