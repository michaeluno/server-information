...
class ServerInformation_AdminPageFramework_FieldType_size extends ServerInformation_AdminPageFramework_FieldType_select {
    ...
    protected function getStyles() {
        return <<<CSSRULES
/* Size Field Type */
.server-information-field-size input {
    text-align: right;
}
.server-information-field-size select.size-field-select {
    vertical-align: 0px;     
}
.server-information-field-size label {
    width: auto;     
} 
.form-table td fieldset .server-information-field-size label {
    display: inline;
}
CSSRULES;
    ...
    protected function _getFieldOutputByLabel($isKey, $sLabel, array $aField) {
        ...
        $_aOutput = array($this->getElementByLabel($aField['before_label'], $isKey, $aField['label']), "<div class='server-information-input-label-container server-information-select-label' style='min-width: " . $this->sanitizeLength($aField['label_min_width']) . ";'>", $this->_getNumberInputPart($aField, $_aBaseAttributes, $isKey, $aField['label']), $this->_getUnitSelectInput($aField, $_aBaseAttributes, $isKey, $aField['label']), "</div>", $this->getElementByLabel($aField['after_label'], $isKey, $aField['label']));
        return implode('', $_aOutput);
    }
    private function _getNumberInputPart(array $aField, array $aBaseAttributes, $isKey, $bMultiLabels) {
        ...
        $_sLabel = $this->getElementByLabel($aField['label'], $isKey, $aField['label']);
        return "<label " . $this->getAttributes($_aSizeLabelAttributes) . ">" . $this->getElement($aField, $bMultiLabels ? array('before_label', $isKey, 'size') : array('before_label', 'size')) . ($aField['label'] && !$aField['repeatable'] ? "<span class='server-information-input-label-string' style='min-width:" . $this->sanitizeLength($aField['label_min_width']) . ";'>" . $_sLabel . "</span>" : "") . "<input " . $this->getAttributes($_aSizeAttributes) . " />" . $this->getElement($aField, $bMultiLabels ? array('after_input', $isKey, 'size') : array('after_input', 'size')) . "</label>";
    }
    private function _getUnitSelectInput(array $aField, array $aBaseAttributes, $isKey, $bMultiLabels) {
        ...
        $_oUnitInput = new ServerInformation_AdminPageFramework_Input_select($_aUnitAttributes + array('select' => $_aUnitAttributes));
