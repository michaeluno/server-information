 ...
 */
class ServerInformation_AdminPageFramework_Form_View___Script_RepeatableField extends ServerInformation_AdminPageFramework_Form_View___Script_Base {
...
(function ( $ ) {
     ...
     */
    $.fn.updateServerInformation_AdminPageFrameworkRepeatableFields = function( aSettings ) {
        ...
        if ( ! $.fn.aServerInformation_AdminPageFrameworkRepeatableFieldsOptions ) {
            $.fn.aServerInformation_AdminPageFrameworkRepeatableFieldsOptions = [];
        }
        if ( ! $.fn.aServerInformation_AdminPageFrameworkRepeatableFieldsOptions.hasOwnProperty( sFieldsContainerID ) ) {     
            $.fn.aServerInformation_AdminPageFrameworkRepeatableFieldsOptions[ sFieldsContainerID ] = $.extend({    
        ...
        var _aOptions = $.fn.aServerInformation_AdminPageFrameworkRepeatableFieldsOptions[ sFieldsContainerID ];
        ...
        $( nodeThis ).find( '.server-information-repeatable-field-buttons' ).attr( 'data-max', _aOptions[ 'max' ] );
        $( nodeThis ).find( '.server-information-repeatable-field-buttons' ).attr( 'data-min', _aOptions[ 'min' ] );
        $( nodeThis ).find( '.server-information-repeatable-field-buttons' ).attr( 'data-fadein', _aOptions[ 'fadein' ] );
        $( nodeThis ).find( '.server-information-repeatable-field-buttons' ).attr( 'data-fadeout', _aOptions[ 'fadeout' ] );
        ...
        $( nodeThis ).find( '.repeatable-field-add-button' ).click( function() {
            $( this ).addServerInformation_AdminPageFrameworkRepeatableField();
        ...
        $( nodeThis ).find( '.repeatable-field-remove-button' ).click( function() {
            $( this ).removeServerInformation_AdminPageFrameworkRepeatableField();
            return false; // will not click after that
        });     
        
        /* If the number of fields is less than the set minimum value, add fields. */
        var _sFieldID           = nodeThis.find( '.repeatable-field-add-button' ).first().closest( '.server-information-field' ).attr( 'id' );
        var _nCurrentFieldCount = jQuery( '#' + sFieldsContainerID ).find( '.server-information-field' ).length;
        if ( _aOptions[ 'min' ] > 0 && _nCurrentFieldCount > 0 ) {
            if ( ( _aOptions[ 'min' ] - _nCurrentFieldCount ) > 0 ) {     
                $( '#' + _sFieldID ).addServerInformation_AdminPageFrameworkRepeatableField( _sFieldID );  
            }
        }
     ...
     */
    $.fn.addServerInformation_AdminPageFrameworkRepeatableField = function( sFieldContainerID ) {
        
        if ( 'undefined' === typeof sFieldContainerID ) {
            var sFieldContainerID = $( this ).closest( '.server-information-field' ).attr( 'id' );    
        }

        var nodeFieldContainer  = $( '#' + sFieldContainerID );
        var nodeNewField        = nodeFieldContainer.clone(); // clone without bind events.
        var nodeFieldsContainer = nodeFieldContainer.closest( '.server-information-fields' );
        var sFieldsContainerID  = nodeFieldsContainer.attr( 'id' );

        // If the set maximum number of fields already exists, do not add.
        if ( ! $.fn.aServerInformation_AdminPageFrameworkRepeatableFieldsOptions.hasOwnProperty( sFieldsContainerID ) ) {     
            var nodeButtonContainer = nodeFieldContainer.find( '.server-information-repeatable-field-buttons' );
            $.fn.aServerInformation_AdminPageFrameworkRepeatableFieldsOptions[ sFieldsContainerID ] = {    
        ...
        var _iFadein  = $.fn.aServerInformation_AdminPageFrameworkRepeatableFieldsOptions[ sFieldsContainerID ][ 'fadein' ];
        var _iFadeout = $.fn.aServerInformation_AdminPageFrameworkRepeatableFieldsOptions[ sFieldsContainerID ][ 'fadeout' ];

        // Show a warning message if the user tries to add more fields than the number of allowed fields.
        var sMaxNumberOfFields = $.fn.aServerInformation_AdminPageFrameworkRepeatableFieldsOptions[ sFieldsContainerID ]['max'];
        if ( sMaxNumberOfFields != 0 && nodeFieldsContainer.find( '.server-information-field' ).length >= sMaxNumberOfFields ) {
            var nodeLastRepeaterButtons = nodeFieldContainer.find( '.server-information-repeatable-field-buttons' ).last();
         ...
         * */
        nodeNewField.updateServerInformation_AdminPageFrameworkRepeatableFields();
        ...
        // For nested fields,
        $( nodeNewField ).find( '.server-information-field' ).each( function( iIterationIndex ) {    
    ...
    $.fn.removeServerInformation_AdminPageFrameworkRepeatableField = function() {
        
        /* Need to remove the element: the field container */
        var nodeFieldContainer  = $( this ).closest( '.server-information-field' );
        var nodeFieldsContainer = $( this ).closest( '.server-information-fields' );
        var sFieldsContainerID  = nodeFieldsContainer.attr( 'id' );
        
        /* If the set minimum number of fields already exists, do not remove */
        var sMinNumberOfFields  = $.fn.aServerInformation_AdminPageFrameworkRepeatableFieldsOptions[ sFieldsContainerID ][ 'min' ];
        if ( sMinNumberOfFields != 0 && nodeFieldsContainer.find( '.server-information-field' ).length <= sMinNumberOfFields ) {
            var nodeLastRepeaterButtons = nodeFieldContainer.find( '.server-information-repeatable-field-buttons' ).last();
            ...
            var _iFadeout = $.fn.aServerInformation_AdminPageFrameworkRepeatableFieldsOptions[ sFieldsContainerID ][ 'fadeout' ]
                ? $.fn.aServerInformation_AdminPageFrameworkRepeatableFieldsOptions[ sFieldsContainerID ][ 'fadeout' ]
                : 500;
            nodeMessage.delay( 2000 ).fadeOut( _iFadeout );
            return;     
        }     
